...
其它源码中出现的 `PING`、`TEST`、`BEEP`、`TARE`、`STAT`、`QR`、`REMIND`、`ALERT`、`REC` 属于心跳、调试或设备内部事件，不作为跨模块业务 API。
...
协议采用 CRLF 分行文本格式，便于调试与跨平台解析；调用方应处理无回执、格式错误和设备断连等异常场景。
