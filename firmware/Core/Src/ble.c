...
void BLE_Init(void)
{
    ...
    /* ---- 使能 NVIC 中断通道（必须！否则 IRQHandler 永远不触发）---- */
...
int BLE_Config(void)
{
    ...
    if (BLE_SendAT("AT", "OK", 500) != 0) {
        /* 可能是模块已连接或未就绪，再稍等重试一次 */
        HAL_Delay(500);
        if (BLE_SendAT("AT", "OK", 500) != 0)
            return -1;
    }

    /* 2. 设置广播名称（默认是 JDY-31-SPP） */
    BLE_SendAT("AT+NAMESmartPillbox", "OK", 500);

    /* 3. 确认当前波特率 */
    BLE_SendAT("AT+BAUD", "OK", 200);
...
{
    /* 如果 30 秒内没收到任何数据（含心跳），认为断连 */
    if (HAL_GetTick() - ble_last_rx_tick > 30000)
